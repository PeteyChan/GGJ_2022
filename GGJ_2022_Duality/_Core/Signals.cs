...
public static class Signals
{
    ...
    class SignalRelay<Args> : Godot.Node
    {
    ...
    static Emitter Connect<Emitter, Args>(string notification, Emitter emitter, in System.Action<Args> action) where Emitter : Godot.Node
    {
        foreach (Godot.Node child in emitter.GetChildren())
            ...
            {
                ((SignalRelay<Args>)child).action += action;
                return emitter;
            }

        var relay = new SignalRelay<Args>();
        relay.action += action;
    ...
    static Emitter Connect<Emitter>(string notification, Emitter emitter, in System.Action action) where Emitter : Godot.Node
    {
        foreach (Godot.Node child in emitter.GetChildren())
            if (child.Name == notification)
            {
                ((SignalRelay)child).action += action;
                return emitter;
            }

        var relay = new SignalRelay();
        relay.Name = notification;
        relay.action += action;
        emitter.AddChild(relay);
        emitter.Connect(notification, relay, "Invoke");
    ...
    public static void OnCallback<T>(this Godot.Node node, in System.Action<T> callback)
    {
        if (node.IsValid())
        {
    ...
    public static void RemoveCallback<T>(this Godot.Node node, in System.Action<T> callback)
    {
        if (node_event_handlers.TryGetValue(node, out var handler))
            handler.UnRegister(callback);
    }

    public static void Send<T>(this Godot.Node node, in T args, bool propogate_up = false)
    {
        if (node_event_handlers.TryGetValue(node, out var handler))
            handler.Invoke(args);
        ...
        if (propogate_up)
            if (node.IsValid())
                Send(node.GetParent(), args, propogate_up);
    }
