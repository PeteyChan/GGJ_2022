...
public struct Debug
{
    ...
    /// <summary>
    /// Asserts that condition is true else throws an exception.
    /// Assert functions are stripped during release build
    /// </summary>
    [System.Diagnostics.Conditional("DEBUG")]
    public static void Assert(bool condition, string fail_message, int logged_stack_frame = 0)
    {
        logged_stack_frame = logged_stack_frame.min(0);

        if (string.IsNullOrEmpty(fail_message))
            fail_message = "Failed Assertion";

        if (!condition)
            throw new AssertException(fail_message);
    }
    ...
    /// <summary>
    /// Asserts that all conditions are true else throws an exception.
    /// Assert functions are stripped during release build
    /// </summary>
    [System.Diagnostics.Conditional("DEBUG")]
    public static void Assert(params bool[] conditions)
    {
        for (int i = 0; i < conditions.Length; ++i)
            Assert(conditions[i], $"Condition {i + 1} failed", 1);
    }
    ...
    static void RunTests(Command args)
    {
                    ...
                    {
                        {
                            ...
                            System.Diagnostics.StackFrame source, target = null;
                            var frames = new System.Diagnostics.StackTrace(exception, true).GetFrames();
                            source = frames[0];
                            for (int i = 1; i < frames.Length; ++i)
                            {
                                if (frames[i].GetMethod()?.Name == method.Name)
                                {
                                    target = frames[i];
                                }
                            }
                            ...
                            string file_name = source.GetFileName()?.Replace(System.IO.Directory.GetCurrentDirectory(), "");
                            if (exception is AssertException assert)
                            {
                                Debug.Log("-> Condition:", assert.Message.Replace("Condition", "").Replace("failed", ""));
                                Debug.Log("-> Type:    Failed Assertion");
                            }
                            else
                            {
                                Debug.Log("-> Line:   ", source.GetFileLineNumber());
                                Debug.Log("-> Method: ", source.GetMethod().Name);
                                Debug.Log("-> Source: ", file_name);
                                Debug.Log("-> Type:   ", exception.GetType().Name);
                            }
                            Debug.Log("FAILED", calling_method, "at line", target.GetFileLineNumber());
                        }
                    }
                    Debug.Log("");
                }
        ...
        Debug.Log("Tests Passed: ", pass, "/", count);
